...
class LogParser {
    ...
    def processFile(logpath) {
        ...
        def outpath = Path.of(logpath.fileName.toString() + ".sql")
                ...
                if (fields) {
                    ...
                    if (! (req in ignoreURLs) && ! (code in ignoreRespCodes) && ms >= minRespTime) {
                        ...
                        if (! statContent(url) && url.startsWith(webAppName)) {
                            ...
                            // output SQL statement with correct formatting
                            // - need to add TZ to timestamp literal?
                            outw.println "INSERT INTO access_log VALUES ('$timestamp', '$op', $code, $ms, $length, '$url');"
                        }
                    }
                } else {
                    println "what? $line"
